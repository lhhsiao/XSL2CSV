...
public class NGCCalanderReader {
...
//				Sheet sheet = w.getSheet(i);
			...
			if(sheet.length>0){
				tempEvents = parseACanlander(sheet[i],
...
//					temp.setEngName(names[1]);
				...
				if(time_str.length()>5 || time_str.length()<1)
					return null;
				time_str=time_str.length()>4?time_str:"0"+time_str;
				time_str=time_str.replaceAll("00:", "24:");
...
//		}
	...
	private String detectMonthOfCanlander(String[][] sheet) {//TODO
		...
		if(sheet.length>0)
		if(sheet[0].length>3){
			String str=sheet[1][4];//sheet.getCell(0,2).getContents();
...
//				System.out.println(sdf.format(new Date()));
				...
				String date=replaceMonthbyNum(str)+"/"+y;
				try {
